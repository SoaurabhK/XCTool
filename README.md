...
3.  ```Constants``` enum contains constants for executable paths, command-line arguments for ```xcresulttool``` and ```xcodebuild```, project-path and scheme-name.
4.  ```ActionsInvocationRecord``` struct contains ```actions```, ```metadataRef``` and ```metrics```. We extract ```testsRefId``` from this nested data-model.
5.  ```ActionTestPlanRunSummaries``` struct contains ```summaries```. We extract ```summaryRefIds``` correspoding to each test from this data-model.
6.  ```ActionTestSummary``` struct contains ```performanceMetrics```, ```duration```, ```identifier```, ```name``` and ```testStatus```. We log this data-model corresponding to each performance test.
7.  ```REPLExecutor``` struct takes ```REPLCommand``` as input and executes that command. It also formats/decode the output as needed.
8.  ```REPLCommand``` struct takes ```launchPath``` and ```arguments``` as input. It has ```run``` methods which launch a ```Process``` with given inputs and attach a ```Pipe``` for reading standardOutput.
9.   ```REPLBuffer``` struct temporarily holds  ```Data``` buffer as it gets streamed from standardOutput. It has mutating methods to ```append``` data in buffer and get ```outstandingText``` from buffer.
